...
void setMotorPosition(double *position_set)
{
    for (int id = 0; id < MOTOR_NUM; id++)
    {
        WbDeviceTag motor = wb_robot_get_device(motors_c[id]);
        wb_motor_set_position(motor, position_set[id]);
    }
}

void setTailMotorPosition(double *position_set)
{
    WbDeviceTag tail = wb_robot_get_device(motors_c[4]);
    wb_motor_set_position(tail, *position_set);
}
...
void setMotorTorque(double *torque_set, unsigned int mode)
{
    ...
    {
        for (int id = 0; id < MOTOR_NUM - 1; id++) // 把尾巴电机给去掉
        {
            WbDeviceTag motor = wb_robot_get_device(motors_c[id]);
