...
Quaternion::Quaternion(const float w[3], const float dt)
{
    ...
    if (theta > 0.03f)
    {
        ...
        this->x = (w[0] / norm_w) * sin;
        this->y = (w[1] / norm_w) * sin;
        this->z = (w[2] / norm_w) * sin;
    }
    else
    {
        this->x = w[0] * (dt / 2);
...
void Quaternion::toEulerAngle(volatile float *roll, volatile float *pitch,
